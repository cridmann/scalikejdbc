...
trait DBSession extends LogSupport with LoanPattern {
  ...
  private def createStatementExecutor(conn: Connection, template: String, params: Seq[Any],
    returnGeneratedKeys: Boolean = false, keyToReturn: Option[String] = None, userMustSpecifyKeyName: Boolean = false): StatementExecutor = {
    try {
      val statement = {
        if (returnGeneratedKeys) {
          if (userMustSpecifyKeyName) {
            keyToReturn map { key =>
              conn.prepareStatement(template, Seq(key).toArray)
            } getOrElse conn.prepareStatement(template, Statement.RETURN_GENERATED_KEYS) // will throw an error
          } else {
  ...
  def updateWithFilters(before: (PreparedStatement) => Unit,
    after: (PreparedStatement) => Unit,
    template: String,
    params: Any*): Int = updateWithFilters(false, before, after, template, None, false, params: _*)
  ...
  def updateWithFilters(returnGeneratedKeys: Boolean,
    ...
    userMustSpecifyKeyName: Boolean,
    params: Any*): Int = {
    ensureNotReadOnlySession(template)
    using(createStatementExecutor(
      ...
      userMustSpecifyKeyName = userMustSpecifyKeyName)) {
      executor =>
        before(executor.underlying)
  ...
  def updateAndReturnSpecifiedGeneratedKey(template: String, params: Any*)(key: Any): Long = {
    ...
    val userMustSpecifyKey = driverName.exists(dname => GlobalSettings.driverNamesToChooseColumnNameForAutoGeneratedKeyRetrieval.contains(dname))

    key match {
      case k: String =>
        updateWithFilters(true, before, after, template, Some(k), userMustSpecifyKey, params: _*)
      case _ =>
        if (userMustSpecifyKey) throw new IllegalStateException(s"You must specify the key to return for the driver: ${driverName.getOrElse("")}")
        updateWithFilters(true, before, after, template, None, userMustSpecifyKey, params: _*)
    }
...
object DBSession {
  ...
  def apply(conn: Connection, driverName: Option[String], tx: Option[Tx] = None, isReadOnly: Boolean = false): DBSession = {
    ActiveSession(conn, driverName, tx, isReadOnly)
  }
 ...
 */
case class ActiveSession(private[scalikejdbc] val conn: Connection, driverName: Option[String], tx: Option[Tx] = None, isReadOnly: Boolean = false)
    extends DBSession {
